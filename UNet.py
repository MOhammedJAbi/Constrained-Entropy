...
class UNetG(nn.Module):
    def __init__(self, nin, nG, nout):
        # nin: 4 (modlaities)
        #  nG: num. kernels initial: 32/64
        # nout: nombre de classes 
        super(UNetG, self).__init__()
        print('*'*50)
        print('--------- Creating UNet network... ---')
        print('*'*50)
...
class UNet_Original(nn.Module):
    def __init__(self, nin, nG, nout):
        ...
        print('*'*50)
        print('--------- Creating UNet Original network... ---')
        print('*'*50)
        self.conv0 = nn.Sequential(convBatch(nin, nG),
                                   convBatch(nG, nG))
        self.conv1 = nn.Sequential(convBatch(nG * 1, nG * 2, stride=2),
...
class UNetDeepSupervision(nn.Module):
    ...
    def forward(self, input):
        ...
        return [F.softmax(self.final(y3), dim=1), 
                F.softmax(self.out_05(y1), dim=1), 
                F.softmax(self.out_025(y0), dim=1), 
                F.softmax(self.out_0125(y), dim=1)]
        
class UNetG_Dilated(nn.Module):
    def __init__(self, nin, nG, nout):
        super(UNetG_Dilated, self).__init__()
        print('*'*50)
        print('------- Creating UNet with Dilated convolutions ------')
        print('*'*50)
        self.conv0 = nn.Sequential(convBatch(nin, nG,stride=1, dilation=1, padding=1),
                                   convBatch(nG, nG))
        self.conv1 = nn.Sequential(convBatch(nG * 1, nG * 2, stride=2, dilation=1, padding=1),
                                   convBatch(nG * 2, nG * 2, stride=1, dilation=1, padding=1),
                                   convBatch(nG * 2, nG * 2, stride=1, dilation=2, padding=2))
                                   
        self.conv2 = nn.Sequential(convBatch(nG * 2, nG * 4, stride=2, dilation=1, padding=1),
                                   convBatch(nG * 4, nG * 4, stride=1, dilation=1, padding=1),
                                   convBatch(nG * 4, nG * 4, stride=1, dilation=4, padding=4))
                                   
        self.conv3 = nn.Sequential(convBatch(nG * 4, nG * 8, stride=2, dilation=1, padding=1),
                                   convBatch(nG * 8, nG * 8, stride=1, dilation=1, padding=1),
                                   convBatch(nG * 8, nG * 8, stride=1, dilation=8, padding=8))
...
class UNetG_Dilated_NoStride(nn.Module):
    def __init__(self, nin, nG, nout):
        super(UNetG_Dilated_NoStride, self).__init__()
        print('*'*50)
        print('------- Creating UNet with Dilated convolutions and No stride ------')
        print('*'*50)
 
        self.conv0 = nn.Sequential(convBatch(nin, nG,stride=1, dilation=2, padding=2),
                                   convBatch(nG, nG))
        self.conv1 = nn.Sequential(convBatch(nG * 1, nG * 2, stride=1, dilation=2, padding=2),
                                   convBatch(nG * 2, nG * 2))
        self.conv2 = nn.Sequential(convBatch(nG * 2, nG * 4, stride=1, dilation=2, padding=2),
                                   convBatch(nG * 4, nG * 4))
        self.conv3 = nn.Sequential(convBatch(nG * 4, nG * 8, stride=1, dilation=2, padding=2),
                                   convBatch(nG * 8, nG * 8))
        self.bridge = nn.Sequential(convBatch(nG * 8, nG * 16, stride=1, dilation=2, padding=2),
                                    residualConv(nG * 16, nG * 16),
                                    convBatch(nG * 16, nG * 8))

        # If we employ concatenations
        #self.deconv0 = upSampleConv(nG * 16, nG * 16)
        '''self.conv4 = nn.Sequential(convBatch(nG * 24, nG * 8),
                                   convBatch(nG * 8, nG * 8))
        #self.deconv1 = upSampleConv(nG * 8, nG * 8)
        ...
        # If we employ residual connections
        self.conv4 = nn.Sequential(convBatch(nG * 8, nG * 8),
                                   convBatch(nG * 8, nG * 8),
                                   convBatch(nG * 8, nG * 4))
                                   
        self.conv5 = nn.Sequential(convBatch(nG * 4, nG * 4),
                                   convBatch(nG * 4, nG * 4),
                                   convBatch(nG * 4, nG * 2))
        
        self.conv6 = nn.Sequential(convBatch(nG * 2, nG * 2),
                                   convBatch(nG * 2, nG * 2),
                                   convBatch(nG * 2, nG * 1))
        
        self.conv7 = nn.Sequential(convBatch(nG * 1, nG * 1))
    ...
    def forward(self, input):
        ...
        # If we employ concatenations
        '''y = self.conv4(torch.cat((bridge, x3), dim=1))
        y = self.conv5(torch.cat((y, x2), dim=1))
        y = self.conv6(torch.cat((y, x1), dim=1))
        y = self.conv7(torch.cat((y, x0), dim=1))'''
        
        # If we employ residual connections
        y = self.conv4((bridge + x3)/2)
        y = self.conv5((y + x2)/2)
        y = self.conv6((y + x1)/2)
        y = self.conv7((y + x0)/2)
